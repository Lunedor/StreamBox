...
// Modify createMediaCard to include available icon and rank
function createMediaCard(item, type, rank = null) {
  // Handle "Show All" card
  if (item.is_show_all) {
    const card = document.createElement("div");
    card.className = "media-card show-all-card";
    ...
    card.innerHTML = `
        <div class="show-all-wrapper">
            ...
            <span class="show-all-label">View All</span>
        </div>
    `;

    card.onclick = () => {
      const filterParam = item.list_type === 'favorites' ? 'favorites' :
        item.list_type === 'watched' ? 'watched' : 'all';
      window.location.href = `filter.html?filter=${filterParam}&type=${type}`;
    };
    return card;
  }
  ...
  card.innerHTML = `
    ...
    ${rank ? `<span class="rank-number">${rank}</span>` : ''}
  ...
  switch (event.key) {
    ...
    case 'm':
      ...
      if (window.handleNavigation) {
        window.handleNavigation('movies');
        // Update active state in nav
        document.querySelectorAll('.nav-link').forEach(l => l.classList.remove('active'));
        const movieLink = document.querySelector('.nav-link[data-action="movies"]');
    ...
    case 't':
      ...
      if (window.handleNavigation) {
        window.handleNavigation('tv');
        // Update active state in nav
        document.querySelectorAll('.nav-link').forEach(l => l.classList.remove('active'));
        const tvLink = document.querySelector('.nav-link[data-action="tv"]');
    ...
    case 'Escape':
      ...
      // Close search modal specifically
      ...
      if (hamburgerDropdown && hamburgerDropdown.classList.contains('open')) {
